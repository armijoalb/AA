...
```{r}
trans2 = caret::preProcess(credit_card.train, c("BoxCox", "scale", "center") )
...
```{r}
bt7 = ada::ada(formula = default.payment.next.month~PAY_1+PAY_2+BILL_AMT1+LIMIT_BAL+AGE+ed.school+PAY_AMT1+PAY_5+marriage.married,data=trainTransformado2, iter=50)
pred.bt7 = predict(bt7, trainTransformado2)
Ein.bt7 = mean( pred.bt7 != trainTransformado2$default.payment.next.month)
Ein.bt7

predTs.bt7 = predict(bt7, testTransformado2)
Eout.bt7 = mean( predTs.bt7 != testTransformado2$default.payment.next.month)
Eout.bt7
```
...
```{r}
print(table(pred.bt3, trainTransformado$default.payment.next.month))
```

```{r}
salida.boosting = vector(length = length(predTs.bt3))
cero = grep("0", predTs.bt3)
...
#nn1 = neuralnet(formula = default.payment.next.month~PAY_1+PAY_2+BILL_AMT1+LIMIT_BAL+AGE+ed.school+PAY_AMT1+PAY_5+marriage.married,data=train_)
nn1 = neuralnet(formula = default.payment.next.month~PAY_1+PAY_2+BILL_AMT1+LIMIT_BAL+AGE+ed.school+PAY_AMT1+PAY_5+marriage.married,data=train_, hidden = 3)
...
nn1.test = neuralnet(formula = default.payment.next.month~PAY_1+PAY_2+BILL_AMT1+LIMIT_BAL+AGE+ed.school+PAY_AMT1+PAY_5+marriage.married,data=test_, hidden = 3)
...
Para crear los modelos del Random Forest, utilizaremos la biblioteca randomForest, que contiene la función randomForest 

```{r}

rf = randomForest(formula = default.payment.next.month~PAY_1+PAY_2+BILL_AMT1+LIMIT_BAL+AGE+ed.school+PAY_AMT1+PAY_5+marriage.married, data = trainTransformado, y = trainTransformado$default.payment.next.month, ntree = 1000, maxnodes=200)
rf
